...
Models and Stores both support snapshots. Model snapshots represent durable
domain state. Store snapshots include explicit `@state` session/view fields and
keyed reactive children, and are hydrated with
`mount(createStore(Type), { snapshot })` before lifecycle effects activate.
