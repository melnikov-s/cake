...
| 4. Model Presets slice    | Complete    | `PiModels` now owns the process-scoped session-less catalog, authentication/availability projection, supported thinking levels, Fast support, exact no-fallback resolution, refresh, and bounded completion. Free `modelPresets.ts` Effects perform semantic CRUD/default/resolve operations through transactional `ApplicationState`; focused Effect RPC and the generated client replaced legacy `set-model-presets` and `list-models`. At Phase 4 completion, the r-state-tree Store hydrated through `modelPresets.list`, serialized optimistic semantic commands, preserved unresolved presets, and no longer read presets from broad Application projection; Packet 7A has now replaced that renderer path with `RendererLive` and effect-state-tree. Cake agents receive the current validated preset/default projection through `models.list`, not raw storage instructions. Packet 5A moved all transcript-free utility completion callers behind `PiModels.complete` and removed `src/agent/utility-model.ts` while preserving plugin-specific fallback policy. The live-session catalog projection used by `src/agent/cake-runtime.ts` and live-runtime portions of `refreshModelsEverywhere` remain for the Phase 5B `PiSessions` boundary, not as another model authority.                  |
| 5. Pi Services            | In progress | Packet 5A completed the process-scoped `PiModels` boundary: session title generation, fallback Managed Worktree naming, context-free composer rewording, and plugin bounded completions now use `PiModels.complete`; the deleted `src/agent/utility-model.ts` no longer creates parallel `ModelRuntime` instances. Plugin utility/default/current fallback remains observable Cake policy and execution never falls back after provider work begins. `PiAgentResources` now loads and explicitly reloads Schema-validated, context-dependent skill, prompt-template, configured extension-source, package-source, and diagnostic snapshots through Pi; executable commands/tools remain session-bound and are intentionally absent. The temporary live-session compatibility projection moved beneath `src/services/pi` pending Packet 5B. Packet 5B starts at `createCakeRuntime` in `src/agent/cake-runtime.ts`, with `PiWorkspaceDriver` and `GlobalChatDriver` as runtime owners: implement keyed scoped `PiSessions`, move session discovery/projection, isolated/sidecar runtime construction (including project-aware rewording), extension UI binding, retry/recovery, and remaining ordinary Pi imports beneath `src/services/pi`, then remove the replaced adapters and driver lifecycle maps. |
| 6. Cake Session domain    | Not started | Unify Project/Cake Chat/Discussion/Subagent behavior above `PiSessions`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 7. Renderer state tree    | In progress | Packet 7A established one window-scoped `RendererLive`/`ManagedRuntime`, mounts effect-state-tree outside React under an owned Scope, and provides the raw `ModelPresetSettingsStore` through the Effect React provider. Model Preset entities and their collection are validated renderer Models; the Store acquires `CakeIpcClient` synchronously, hydrates through a post-publication autorun, serializes optimistic semantic commands with a scoped semaphore, preserves unresolved presets, and disposes Models and operations with its Scope. The Model Preset operations were removed from the temporary Promise `DesktopClient` facade. A narrow revision-only bridge invalidates unmigrated r-state-tree consumers without duplicating Model Preset authority; remove it when those consumers migrate. Existing manual window persistence remains until Phase 8. The local package passes its own typecheck and 99 tests, but Cake's full typecheck remains blocked because the file-linked package exports TypeScript source that does not compile under Cake's `noUncheckedIndexedAccess`; resolve that package boundary before Packet 7B. Packet 7B starts with Project and Session catalog Models/Stores after Packet 6A provides their query/subscription RPC boundary.                    |
