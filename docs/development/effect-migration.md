...
| 2. Effect RPC             | Not started | Start at `src/preload/preload.ts` and the `cake:request` handler in `src/main/main.ts`: add the Electron RPC transport, then remove `get-home-directory` from `desktop-ipc` as the first real migrated query while proving query/error/stream/cancel. |
