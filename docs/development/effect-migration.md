...
| 3. Typed storage          | Not started | Begin at `loadApplicationState`/`persistApplicationState` in `src/main/main.ts` and `src/models/Application.ts`: replace the `application.json` read/write path with versioned `ApplicationStorage`, then route the first renderer application-state operation through Effect RPC.                                                                                                                                                                                                       |
