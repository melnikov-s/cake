...
| Project history, trusted paths, per-session composer drafts, pending project-session identities, staged draft-session messages and attachments, theme, reasoning visibility | Cake main process                 | Atomic `window-state.json`, saved only after renderer hydration                                    |
