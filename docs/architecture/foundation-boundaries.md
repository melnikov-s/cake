...
the grouped `CakeIpcClient`; focused window-owned observers consume scoped
Streams through the renderer runtime's common observation primitive. The Model
observer applies authoritative snapshots and reduces subsequent Events
...
Promise-based `Client`. RPC handlers are thin adapters to main domain
...
- Focused Stores own renderer workflows, local operations, and concurrency
  policy. Renderer bootstrap attaches focused window-owned observers and
  one-way Store snapshot persistence to the mounted Root Store; none of these
  infrastructure owners participates in the Store tree.
