...
the grouped `CakeIpcClient`; projection synchronizers consume scoped Streams and
reduce Updates into renderer-owned r-state-tree Models. Ordinary Stores use the
...
- Focused Stores own renderer workflows, local operations, and concurrency
  policy; projection synchronizers own authoritative Stream subscriptions.
