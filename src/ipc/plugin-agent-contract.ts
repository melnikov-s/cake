...
export const agentSessionTargetSchema = z.discriminatedUnion("kind", [
  z.object({ kind: z.literal("new"), workspace: workspaceRefSchema.optional(), visibility: z.enum(["private", "project"]).default("private") }),
  z.object({ kind: z.literal("attach"), target: sessionRefSchema.optional() }),
  z.object({ kind: z.literal("fork"), source: sessionRefSchema.optional(), entryId: z.string().min(1).max(256).optional(), visibility: z.enum(["private", "project"]).default("private") })
]);
...
export const agentModelPreferenceSchema = z.discriminatedUnion("prefer", [
  ...
  z.object({ prefer: z.literal("exact"), provider: z.string().min(1).max(256), modelId: z.string().min(1).max(512), thinkingLevel: thinkingLevelSchema.optional() })
]);
...
export const sessionContextSelectionSchema = z.union([
  ...
  z.object({ kind: z.literal("current-branch"), maximumInputCharacters: z.number().int().min(1).max(PLUGIN_COMPLETION_INPUT_MAX) })
]);

export const pluginCompletionRequestSchema = z.object({
  model: agentModelPreferenceSchema.default({ prefer: "utility" }),
  context: z.object({ kind: z.literal("session"), target: sessionRefSchema.optional(), selection: sessionContextSelectionSchema }),
