...
export const compiledInlineWidgetSchema = z.object({
  url: z.url().startsWith("cake-widget://document/"),
  token: z.uuid()
}).refine((widget) => widget.url === `cake-widget://document/${widget.token}`, { message: "Inline widget URL must match its capability token", path: ["url"] });
