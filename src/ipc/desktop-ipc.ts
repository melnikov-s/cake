...
export const desktopEventSchema = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("global-chat-snapshot"), requestId: z.uuid().optional(), sessionId: z.string().max(256), parts: ipcProjectionArray(uiPartSchema, 100_000), streaming: z.boolean() }),
  z.object({ type: z.literal("global-chat-part-updated"), part: uiPartSchema }),
  z.object({ type: z.literal("global-chat-part-removed"), partId: z.string().max(256) }),
...
export const desktopRequestSchema = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("open-global-chat"), requestId: z.uuid(), tools: z.array(z.object({ name: z.string().min(1).max(256), description: z.string().min(1).max(2_048) })).min(1).max(50) }),
  ...
  z.object({ type: z.literal("clear-global-chat"), requestId: z.uuid(), tools: z.array(z.object({ name: z.string().min(1).max(256), description: z.string().min(1).max(2_048) })).min(1).max(50) }),
