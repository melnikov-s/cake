...
export const desktopRequestSchema = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("submit-review-thread"), requestId: z.uuid(), workspacePath: z.string().max(4_096), sessionId: z.string().max(256), threadId: z.string().min(1).max(256), model: z.object({ provider: z.string().max(256), id: z.string().max(512) }).optional(), thinkingLevel: thinkingLevelSchema.optional() }),
  ...
  z.object({ type: z.literal("inspect-changes"), requestId: z.uuid(), workspacePath: z.string().max(4_096), sessionId: z.string().max(256), source: changeSourceSchema, turnId: z.string().min(1).max(256).optional() }),
