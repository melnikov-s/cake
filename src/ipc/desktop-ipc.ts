...
export const desktopEventSchema = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("changes-snapshot"), requestId: z.uuid(), workspacePath: z.string().max(4_096), sessionId: z.string().max(256), source: changeSourceSchema, selectedTurnId: z.string().min(1).max(256).optional(), turns: ipcProjectionArray(changeTurnSchema, 10_000), files: ipcProjectionArray(changedFileSchema, 10_000) }),
...
export const desktopRequestSchema = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("repair-inline-widget"), sessionId: z.string().min(1).max(256), language: inlineWidgetLanguageSchema, capability: inlineWidgetCapabilitySchema, source: inlineWidgetSourceSchema, context: z.string().max(262_144), diagnostic: z.string().max(32_768).optional(), model: z.object({ provider: z.string().min(1).max(256), id: z.string().min(1).max(512) }).optional() }),
  ...
  z.object({ type: z.literal("create-review-thread"), sessionId: z.string().min(1).max(256), anchor: reviewAnchorSchema, body: z.string().min(1).max(262_144) }),
  z.object({ type: z.literal("reply-review-thread"), sessionId: z.string().min(1).max(256), threadId: z.string().min(1).max(256), body: z.string().min(1).max(262_144) }),
  z.object({ type: z.literal("resolve-review-thread"), sessionId: z.string().min(1).max(256), threadId: z.string().min(1).max(256), resolved: z.boolean() }),
  ...
  z.object({ type: z.literal("resolve-session"), sessionId: z.string().min(1).max(256), resolved: z.boolean() }),
  z.object({ type: z.literal("resolve-sessions"), sessionIds: z.array(z.string().min(1).max(256)).min(1).max(10_000), resolved: z.boolean() }),
  ...
  z.object({ type: z.literal("prompt"), requestId: z.uuid(), text: z.string().max(262_144), delivery: z.enum(["prompt", "steer", "follow-up"]), attachments: z.array(attachmentSchema).max(20), sessionId: z.string().max(256) }).refine((request) => Boolean(request.text.trim() || request.attachments.length), { message: "A prompt requires text or an attachment" }),
  z.object({ type: z.literal("submit-review-thread"), requestId: z.uuid(), sessionId: z.string().max(256), threadId: z.string().min(1).max(256), model: z.object({ provider: z.string().max(256), id: z.string().max(512) }).optional(), thinkingLevel: thinkingLevelSchema.optional() }),
  z.object({ type: z.literal("abort"), requestId: z.uuid(), sessionId: z.string().max(256) }),
  z.object({ type: z.literal("set-model"), requestId: z.uuid(), provider: z.string(), modelId: z.string(), sessionId: z.string().max(256) }),
  z.object({ type: z.literal("set-thinking"), requestId: z.uuid(), level: thinkingLevelSchema, sessionId: z.string().max(256) }),
  z.object({ type: z.literal("set-pi-setting"), requestId: z.uuid(), update: piSettingUpdateSchema, sessionId: z.string().max(256) }),
  z.object({ type: z.literal("reload-pi"), requestId: z.uuid(), sessionId: z.string().max(256) }),
  z.object({ type: z.literal("login"), requestId: z.uuid(), provider: z.string(), authType: z.enum(["api_key", "oauth"]), sessionId: z.string().max(256) }),
  z.object({ type: z.literal("logout"), requestId: z.uuid(), provider: z.string(), sessionId: z.string().max(256) }),
  z.object({ type: z.literal("rename-session"), requestId: z.uuid(), sessionId: z.string().max(256), name: z.string().min(1).max(512) }),
  z.object({ type: z.literal("fork-session"), requestId: z.uuid(), sessionId: z.string().max(256), entryId: z.string().max(256) }),
  z.object({ type: z.literal("navigate-session"), requestId: z.uuid(), sessionId: z.string().max(256), entryId: z.string().max(256) }),
  z.object({ type: z.literal("inspect-changes"), requestId: z.uuid(), sessionId: z.string().max(256), source: changeSourceSchema, turnId: z.string().min(1).max(256).optional() }),
  z.object({ type: z.literal("get-changelog"), requestId: z.uuid(), sessionId: z.string().max(256) }),
  z.object({ type: z.literal("respond-artifact"), requestId: z.uuid(), artifactRequestId: z.uuid(), sessionId: z.string().max(256), value: jsonValueSchema.optional(), cancelled: z.boolean() }),
  ...
  z.object({
    ...
    cancelled: z.boolean(), sessionId: z.string().max(256)
  })
]);
