...
export const SUBAGENT_PROFILE_INSTRUCTIONS = {
  ...
  planner: "Analyze the requested work and return an implementation plan with relevant files, dependencies, risks, and verification. Do not modify files.",
  reviewer: "Review the requested code for correctness, security, and maintainability. Report concrete findings with file and line references. Do not modify files.",
  worker: "Complete the bounded implementation task, verify the result, and summarize changed files and checks. Do not delegate unless the caller explicitly granted delegation depth."
...
export function subagentSystemPrompt(profile: SubagentProfile, instructions?: string) {
  ...
  return instructions ? `${profilePrompt}\n\n## Additional instructions\n\n${instructions}` : profilePrompt;
}
