...
export const pluginIdSchema = z
  .string()
  .regex(/^[a-z0-9]+(?:[.-][a-z0-9]+)+$/, "Plugin IDs must be namespaced lowercase identifiers")
  .max(128);

export const cakePluginManifestSchema = z
  .object({
  ...
  .refine((manifest) => manifest.renderer || manifest.backend || manifest.scene, {
    message: "A plugin must declare a renderer, a backend, a scene, or a combination of them",
  })
  .refine((manifest) => !manifest.activeScene || manifest.scene, {
    message: "Only a plugin with a scene entry may set activeScene",
  })
  .refine((manifest) => !manifest.scene || manifest.scene !== manifest.renderer, {
    message: "Renderer and scene entries must be separate modules",
  });
