...
export async function pastedImageAttachments(files: readonly File[], availableSlots: number) {
  const images = files.filter((file) => file.type.startsWith("image/")).slice(0, Math.max(0, availableSlots));
  return Promise.all(images.map(async (file, index): Promise<Extract<Attachment, { kind: "image" }>> => ({
    kind: "image",
