...
export function usePluginAgent(options: { abortOnUnmount?: boolean } = {}): PluginAgentHandle {
  ...
  useEffect(() => root.client.subscribe((event) => {
    if (event.type === "plugin-agent-event" && event.pluginId === pluginId && event.snapshot.handleId === latest.current?.handleId) setSnapshot(event.snapshot);
  }), [pluginId, root]);

  useEffect(() => () => {
    const current = latest.current;
    if (!current) return;
    if (options.abortOnUnmount && current.status === "running") void root.client.abortPluginAgent(pluginId, current.handleId).finally(() => root.client.detachPluginAgent(pluginId, current.handleId));
    else void root.client.detachPluginAgent(pluginId, current.handleId);
  }, [options.abortOnUnmount, pluginId, root]);

  const open = useCallback(async (input: PluginAgentOpenOptions) => {
    const previous = latest.current;
    if (previous) await root.client.detachPluginAgent(pluginId, previous.handleId);
    setOpening(true); setLocalError(undefined); setSnapshot(undefined);
    try { setSnapshot(await root.client.openPluginAgent(pluginId, input, implicitSession(root))); }
    catch (error) { setLocalError(error instanceof Error ? error.message : String(error)); throw error; }
    finally { setOpening(false); }
  }, [pluginId, root]);

  const deliver = useCallback(async (delivery: "prompt" | "steer" | "follow-up", text: string) => {
    const current = latest.current;
    if (!current) throw new Error("Open the plugin agent before prompting it");
    if (delivery === "prompt" && current.status !== "idle") throw new Error("A normal prompt requires an idle plugin agent");
    setLocalError(undefined);
    try { setSnapshot(await root.client.promptPluginAgent(pluginId, current.handleId, delivery, text)); }
    catch (error) { setLocalError(error instanceof Error ? error.message : String(error)); throw error; }
  }, [pluginId, root]);
  ...
  return useMemo(() => ({
    ...
    status: opening ? "opening" as const : localError ? "error" as const : snapshot?.status ?? "closed" as const,
...
function revision(parts: readonly UiPart[]) {
  let hash = 2166136261;
  for (const character of JSON.stringify(parts.map((part) => [part.id, part.kind, "entryId" in part ? part.entryId : undefined, part.kind === "text" ? part.status : undefined]))) {
...
export function usePluginSessionActivity(): PluginSessionActivity {
  ...
  const initial = (): PluginSessionActivity => {
    ...
    return { streaming: model?.streaming ?? false, settledRevision: revision(parts), leafId: messages.at(-1)?.entryId, lastMessageId: messages.at(-1)?.entryId };
  };
  const [value, setValue] = useState(initial);
  useEffect(() => root.client.subscribe((event) => {
    if (event.type === "session-snapshot-received" && event.snapshot.workspacePath === selection.workspacePath && event.snapshot.sessionId === selection.sessionId) {
      const parts = event.snapshot.parts;
      const messages = parts.filter((part): part is Extract<UiPart, { kind: "text" }> => part.kind === "text" && Boolean(part.entryId));
      setValue((current) => ({ streaming: event.snapshot.streaming, settledRevision: event.snapshot.streaming ? current.settledRevision : revision(parts), leafId: messages.at(-1)?.entryId, lastMessageId: messages.at(-1)?.entryId }));
    } else if (event.type === "streaming-changed" && event.sessionId === selection.sessionId) setValue((current) => ({ ...current, streaming: event.streaming }));
...
export function usePluginCompletion(): PluginCompletionHandle {
  ...
  const [state, setState] = useState<{ status: PluginCompletionHandle["status"]; result?: PluginCompletionResult; error?: string }>({ status: "idle" });
  const abort = useCallback(async () => {
  ...
  useEffect(() => () => { if (active.current) void root.client.cancelPluginCompletion(pluginId, active.current); }, [pluginId, root]);
  const run = useCallback(async (request: PluginCompletionRequest) => {
    if (active.current) await root.client.cancelPluginCompletion(pluginId, active.current);
    const requestId = crypto.randomUUID();
    active.current = requestId;
    setState({ status: "running" });
    try {
      const result = await root.client.runPluginCompletion(pluginId, requestId, request, implicitSession(root));
      if (active.current === requestId) { active.current = undefined; setState({ status: "idle", result }); }
      return result;
    } catch (error) {
      if (active.current === requestId) { active.current = undefined; setState({ status: "error", error: error instanceof Error ? error.message : String(error) }); }
      throw error;
    }
  }, [pluginId, root]);
