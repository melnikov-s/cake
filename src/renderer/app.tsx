...
export const Sidebar = observer(function Sidebar({ store, projects, chat, cakeChat, selection, onOpenSettings, onOpenCakeChat, onCreateCakeChat, onOpenSession, onCreateSession, onStartOneOffChat, onChooseProject, onToggle, onReloadPi }: { store: SidebarStore; projects: ProjectCatalogStore; chat: ProjectWorkbenchStore; cakeChat: GlobalChatStore; selection: AppSelection; onOpenSettings: () => void; onOpenCakeChat(sessionId?: string): void; onCreateCakeChat(): void; onOpenSession(sessionId: string): void; onCreateSession(workspacePath: string): void; onStartOneOffChat(): void; onChooseProject(): void; onToggle: () => void; onReloadPi?: () => void }) {
  ...
  const projectSessionSelected = (sessionId: string) => selection.kind === "project-session" && selection.sessionId === sessionId;
  ...
  const renderProjectGroup = (path: string, resolved: boolean) => {
    ...
    return <div className="project-group" key={path}>
      <div className="project-row" title={path}><button className={`project-disclosure ${collapsed ? "collapsed" : ""}`} type="button" aria-expanded={!collapsed} aria-label={`${collapsed ? "Expand" : "Collapse"} ${projects.nameForPath(path)}${resolved ? " resolved" : ""}`} onClick={() => toggleProject(collapseKey)}><ChevronIcon /></button><button className="project-label" type="button" aria-label={resolved ? `${collapsed ? "Expand" : "Collapse"} ${projects.nameForPath(path)} resolved` : `Start new chat in ${projects.nameFromPath(path)}`} onClick={() => resolved ? toggleProject(collapseKey) : onCreateSession(path)}><FolderIcon /><span>{projects.nameForPath(path)}</span></button>{!resolved && <button className="project-add" aria-label={`New chat in ${projects.nameFromPath(path)}`} onClick={() => onCreateSession(path)}><PlusIcon /></button>}</div>
      {!collapsed && visibleSessions.map((session) => {
        const selected = projectSessionSelected(session.id);
        return <div key={session.id} data-session-id={session.id} className={`session-item ${selected ? "active has-session-action" : ""}`}><button className="session-row" aria-current={selected ? "page" : undefined} onClick={() => onOpenSession(session.id)} onContextMenu={(event) => renameSession(event, session.id, session.title)}><span className="session-title" title={session.title}>{session.title}</span>{!selected && <time className="session-time" dateTime={session.modified} title={new Date(session.modified).toLocaleString()}>{store.sessionActivityTime(session.modified)}</time>}{activityIndicator(session.id)}</button>{selected && <button className="session-resolve-action" type="button" aria-label={`${resolved ? "Restore" : "Resolve"} ${session.title}`} title={resolved ? "Restore" : "Resolve"} onClick={() => void store.setSessionResolved(session.id, !resolved)}>{resolved ? <RestoreIcon /> : <ResolveIcon />}</button>}</div>;
      })}
...
export const App = observer(function App() {
  ...
  return (
    <main className={`app-shell ${sidebarCollapsed ? "sidebar-collapsed" : ""} ${store.commandPane ? "right-pane-open" : ""} ${resizingPanel ? "is-resizing" : ""}`} style={shellStyle}>
      <Sidebar store={sidebar} projects={projects} chat={store} cakeChat={root.globalChatStore} selection={shell.selection} onToggle={() => setSidebarCollapsed((value) => !value)} onOpenSettings={() => root.showSettings()} onOpenCakeChat={(sessionId) => { void root.openCakeChat(sessionId); }} onCreateCakeChat={() => { void root.startCakeChat(); }} onOpenSession={(sessionId) => { void root.openSession(sessionId); }} onCreateSession={(workspacePath) => { void root.createSession(workspacePath); }} onStartOneOffChat={() => { void root.startOneOffChat(); }} onChooseProject={() => { void root.chooseProject(); }} onReloadPi={() => void settings.reloadPi()} />
      ...
      <section className="workspace" data-session-id={shell.selection.kind === "cake-chat" ? shell.selection.sessionId : shell.selection.kind === "project-session" ? shell.selection.sessionId : undefined}>
        ...
        {surface === "settings" ? <SettingsPage store={store} settings={settings} configuration={chatConfiguration} customization={root.customizationStore} onViewStateChange={() => persistence.schedule()} /> : globalChat ? cakeChatSession ? <div className="workbench global-chat"><Chat store={cakeChatSession.chatStore} empty={<div className="chat-empty"><span className="cake-orbit"><span className="cake-mark">C</span></span><h1>What can I help you find or do?</h1><p>Ask about your tasks, open one, or delegate work to it.</p></div>} /></div> : <div className="loading-screen"><span className="cake-mark">C</span><LoadingState label="Opening Cake Chat" /></div> : !session ? (
          ...
          <StoreProvider key={`${session.workspacePath}\u0000${session.sessionId}`} store={session}>
            ...
            <div className="workbench project-session-workbench">
              ...
              <Chat store={session.chatStore} transcriptBehavior={{ onFork: (entryId) => { void store.forkAt(entryId); }, onOpenReviewRun: (threadId) => { void store.openSessionChanges(threadId); }, waitingForUser: Boolean(extensionUi.request || artifactInteractions?.request), messageComments: session.messageCommentsStore, inlineWidgets: root.inlineWidgetStore, artifacts: { records: session.model.artifacts.map((artifact) => artifact.value), interaction: session.artifactInteractionStore } }} empty={<div className="chat-empty"><span className="cake-orbit"><span className="cake-mark">C</span></span><h1>What should we build in <em>{store.projectName}</em>?</h1><p>Describe a task, ask a question, or type <code>/</code> for commands.</p></div>} footer={<><ArtifactsPanel session={session} inlineWidgets={root.inlineWidgetStore} /><Slot name="project-session.transcript.after" /></>} error={chatError ? { message: chatError, details: chatErrorDetails } : undefined} composerContent={<Slot name="project-session.composer.before" />} pluginActions={<Slot name="project-session.composer.actions" />} status={<>{extensionUi.statuses.length > 0 && <div className="extension-statuses" role="status">{extensionUi.statuses.map((status) => <span key={status.key}><strong>{status.key}</strong> {status.text}</span>)}</div>}<Slot name="project-session.status" /></>} />
