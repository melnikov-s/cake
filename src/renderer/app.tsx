  ...
  return (
    <aside className="sidebar">
      ...
      <div className="sidebar-scroll">
        <div className="section-heading"><span>Projects</span><button aria-label="Add project" onClick={() => void store.chooseProject()}><PlusIcon /></button></div>
        {store.recentProjectPaths.length === 0 ? <p className="sidebar-empty">Add a folder to start a project.</p> : store.recentProjectPaths.map((path) => {
          ...
          return <div className="project-group" key={path}>
            <div className={`project-row ${active ? "active" : ""}`}><button className="project-open" title={path} onClick={() => void store.switchProject(path)}><FolderIcon /><span>{store.nameFromPath(path)}</span></button>{active && <button className="project-add" aria-label={`New chat in ${store.nameFromPath(path)}`} onClick={() => void store.startNewSession()}><PlusIcon /></button>}</div>
            {active && store.currentSessions.slice(0, 8).map((session) => <button key={session.id} className={`session-row ${session.id === store.session?.sessionId ? "active" : ""}`} onClick={() => void store.openSession(session.id)}><span>{session.title}</span>{session.id === store.session?.sessionId && store.isStreaming && <i />}</button>)}
          </div>;
        })}
      </div>
  ...
  return (
    <main className="app-shell">
      ...
      <section className="workspace">
        <header className="workspace-header"><div><strong>{store.session ? (store.currentSessions.find((item) => item.id === store.session?.sessionId)?.title || "New chat") : "Cake"}</strong>{store.projectPath && <span>{store.projectPath}</span>}</div>{store.session && <button className="header-new" onClick={() => void store.startNewSession()}><PlusIcon /> New chat</button>}</header>
        {!store.session ? (
          ...
          <div className="chat-layout">
            <div className="transcript"><Conversation>{store.parts.length === 0 ? <div className="chat-empty"><span className="cake-orbit"><span className="cake-mark">C</span></span><h1>What should we build in <em>{store.projectName}</em>?</h1><p>Describe a task, ask a question, or attach a file to get started.</p></div> : groupTranscriptParts(store.parts).map((item) => item.kind === "activity-group" ? <ActivityGroup key={item.id} parts={item.parts} store={store} /> : <TranscriptPart key={item.id} part={item} store={store} />)}{store.error && <div className="notice notice-error" role="alert"><strong>Operation failed</strong><span>{store.error}</span></div>}</Conversation></div>
            <ComposerPanel store={store} />
          </div>
        )}
      </section>
