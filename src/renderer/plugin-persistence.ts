...
function notify(resource: Resource) { resource.revision += 1; for (const listener of resource.listeners) listener(); }

function createResource<T extends SerializablePluginValue>(pluginId: string, key: string, scope: PluginPersistenceScope, schema: z.ZodType<T>, initialValue: T): Resource {
  const resource: Resource = { value: schema.parse(initialValue), version: 0, revision: 0, status: "pending", promise: Promise.resolve(), saveQueue: Promise.resolve(), listeners: new Set() };
  resource.promise = (async () => {
    try {
      ...
      if (response.record) { resource.value = schema.parse(response.record.value); resource.version = response.record.version; }
...
function usePluginState<T extends SerializablePluginValue>(pluginId: string, key: string, scope: PluginPersistenceScope, schema: z.ZodType<T>, initialValue: T): [T, Dispatch<SetStateAction<T>>] {
  const cacheKey = resourceKey(pluginId, key, scope);
  let resource = resources.get(cacheKey);
  if (!resource) { resource = createResource(pluginId, key, scope, schema, initialValue); resources.set(cacheKey, resource); }
...
function isStateUpdater<T extends SerializablePluginValue>(update: SetStateAction<T>): update is (value: T) => T {
  return typeof update === "function";
}

export function usePluginGlobalState<T extends SerializablePluginValue>(pluginId: string, key: string, schema: z.ZodType<T>, initialValue: T) {
  return usePluginState(pluginId, key, { kind: "global" }, schema, initialValue);
}

export function usePluginSessionState<T extends SerializablePluginValue>(pluginId: string, key: string, schema: z.ZodType<T>, initialValue: T) {
