...
export const PluginSettings = observer(function PluginSettings({ store }: { store: CustomizationStore }) {
  ...
  return <>
    <section className="settings-section" aria-labelledby="plugins-title">
      ...
      {slotDiagnostics.length > 0 && <div className="notice notice-error" role="status">
        <strong>Custom scene slot mismatch</strong>
        <span>{slotDiagnostics.map(({ name, outletCount }) => `${name}: ${outletCount === 0 ? "missing" : `${outletCount} outlets`}`).join("; ")}</span>
      </div>}
      {store.plugins.length === 0 ? <p className="settings-empty">No plugins are installed.</p> : <div className="plugin-settings-list">
        {store.plugins.map((plugin) => <article className="plugin-settings-row" key={plugin.id}>
          <div className="plugin-settings-identity">
            ...
            <small>Scene: {plugin.scene ? `${plugin.scene}${plugin.activeScene ? " · Active" : ""}` : "None"}</small>
          ...
          <div className="plugin-settings-actions">
            {plugin.scene && plugin.enabled && <Button variant="outline" size="sm" type="button" disabled={store.busy || Boolean(plugin.diagnostics.length)} onClick={() => void store.setActiveScene(plugin.activeScene ? undefined : plugin.id)}>{plugin.activeScene ? "Use default scene" : "Use scene"}</Button>}
      ...
      <p className="plugin-settings-note">Enabled plugins are trusted local software. Renderer contributions load automatically. A plugin may optionally replace the whole scene, and its scene can still mount Cake slots. Backends may read or modify files, execute programs, and access the network. Changes take effect after validation and activation.</p>
    </section>
