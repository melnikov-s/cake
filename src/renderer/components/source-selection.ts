...
export function selectionColumn(codeElement: HTMLElement, node: Node, offset: number) {
  ...
  const prefixLength =
    codeElement.querySelector<HTMLElement>("[data-review-prefix]")?.textContent?.length ?? 0;
...
export function extractSourceSelection(
  container: HTMLElement,
  rowSelector: string,
  indexAttribute: string,
): NormalizedSourceSelection | undefined {
  ...
  if (!anchorRow || !focusRow || !container.contains(anchorRow) || !container.contains(focusRow))
    return undefined;
  ...
  return {
    ...
    endColumn: selectionColumn(
      endCode,
      endNode,
      forward ? selection.focusOffset : selection.anchorOffset,
    ),
  };
}
