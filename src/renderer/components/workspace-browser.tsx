...
const SourceFile = observer(function SourceFile({ path, store }: { path: string; store: WindowStore }) {
  ...
  const threads = store.reviewThreads.filter((thread) => thread.anchor.view === "file" && thread.anchor.path === path);

  useEffect(() => {
    let active = true;
    setSource(undefined); setTokens(undefined); setError(undefined); setComposer(undefined);
    void store.readWorkspaceFile(path).then((content) => {
      if (!active) return;
      setSource(content);
  ...
  return <div className="change-explorer-diff change-explorer-full-file workspace-source" role="table" aria-label={`Workspace file ${path}`} onMouseUp={selectText}>{lines.map((line, index) => <Fragment key={index}>
    ...
    {composer && !composer.floating && composer.anchor.end.diffLine === index && <ReviewComposer anchor={composer.anchor} onSave={(body) => store.createReviewThread(composer.anchor, body)} onCancel={() => setComposer(undefined)} />}
    {threads.filter((thread) => thread.anchor.end.diffLine === index).map((thread) => <ReviewThreadCard key={`${thread.id}:${thread.status}`} thread={thread} store={store} onFocus={() => store.focusWorkspaceReviewThread(thread.id)} />)}
  </Fragment>)}{composer?.floating && <ReviewComposer anchor={composer.anchor} floating position={composer.position} onSave={(body) => store.createReviewThread(composer.anchor, body)} onCancel={() => setComposer(undefined)} />}</div>;
});
...
export const WorkspaceBrowser = observer(function WorkspaceBrowser({ store }: { store: WindowStore }) {
  ...
  const pendingCount = store.pendingReviewCommentCount;
  return <main className="change-explorer workspace-browser">
    <section className="change-explorer-file"><header><div><small>Project browser · {store.projectName}</small><h1>{path ?? "Select a file"}</h1></div></header>{path ? <SourceFile path={path} store={store} /> : <div className="change-explorer-file-state"><strong>{store.workspaceFilesLoading ? "Loading project…" : "No files to browse"}</strong><span>{store.workspaceFilesLoading ? "Building the project tree." : "This project does not contain any visible files."}</span></div>}</section>
    <aside className="change-explorer-tree"><header><div><strong>Project files</strong><small>{store.workspaceFilesLoading ? "Loading…" : `${store.workspaceFiles.length} files`}</small></div><div className="change-explorer-actions">{pendingCount > 0 && <Button size="sm" onClick={() => void store.sendPendingReviewComments()}>Send ({pendingCount})</Button>}<Button variant="ghost" size="sm" onClick={() => store.closeWorkspaceBrowser()}>Done</Button></div></header><div className="change-explorer-sidebar-body"><nav aria-label="Project files"><ProjectTree nodes={tree.children} selectedPath={path} onSelect={(file) => store.selectWorkspaceFile(file)} /></nav><section className="review-thread-index" aria-label="Code questions"><header><strong>Questions</strong><span>{threads.length}</span></header>{threads.length === 0 ? <p>Select a line or some code to ask Cake about it.</p> : <ol>{threads.map((thread) => <li key={thread.id}><button className={`${thread.status === "resolved" ? "resolved" : ""} ${store.activeReviewThread?.id === thread.id ? "active" : ""}`} onClick={() => store.focusWorkspaceReviewThread(thread.id)}><i className={thread.status === "resolved" ? "resolved" : thread.pending ? "pending" : "replied"} /><span><strong>{threadPreview(thread)}</strong><small>{thread.anchor.path} · L{thread.anchor.start.newLine ?? thread.anchor.start.oldLine}</small></span><em>{thread.status === "resolved" ? "Resolved" : thread.pending ? "Pending" : "Replied"}</em></button></li>)}</ol>}</section></div></aside>
  </main>;
});
