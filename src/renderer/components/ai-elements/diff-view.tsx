...
function parseDiff(diff: string): DiffLine[] {
  ...
  return diff.split("\n").map((line, index) => {
    ...
    if (displayLine) {
      ...
      return { key: `${index}-${line}`, kind, oldNumber: kind === "add" ? undefined : number, newNumber: kind === "remove" ? undefined : number, content: displayLine[3] ?? "" };
    }
    ...
    const parsed = { key: `${index}-${line}`, kind, oldNumber: kind === "add" ? undefined : oldNumber, newNumber: kind === "remove" ? undefined : newNumber, content: marker === "+" || marker === "-" || marker === " " ? line.slice(1) : line } satisfies DiffLine;
...
export function DiffView({ diff, filePath, label = "Changes" }: { diff: string; filePath?: string; label?: string }) {
  const lines = parseDiff(diff);
  ...
  return (
    <section className="diff-view" aria-label={`${label}${filePath ? ` to ${filePath}` : ""}`}>
      <header>
        ...
        <span><b>+{stats.additions}</b><i>−{stats.deletions}</i></span>
      </header>
      <div className="diff-scroll" role="table" aria-label="Code changes">
        {lines.map((line) => line.kind === "meta"
          ? <div className="diff-line diff-meta" role="row" key={line.key}><span /><span /><code>{line.content}</code></div>
          : <div className={`diff-line diff-${line.kind}`} role="row" key={line.key}>
              <span aria-label={line.oldNumber === undefined ? undefined : `Old line ${line.oldNumber}`}>{line.oldNumber}</span>
              <span aria-label={line.newNumber === undefined ? undefined : `New line ${line.newNumber}`}>{line.newNumber}</span>
              <code><b aria-hidden="true">{line.kind === "add" ? "+" : line.kind === "remove" ? "−" : " "}</b>{line.content || " "}</code>
            </div>)}
      </div>
    </section>
  );
