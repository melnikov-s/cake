...
export function Tool({ part }: { part: Extract<UiPart, { kind: "tool" }> }) {
  ...
  return (
    <details className={`tool-call rounded-xl border border-border bg-muted/35 px-4 py-3${diff ? " tool-edit" : ""}`} open={part.state === "error" || (part.state === "running" && Boolean(diff))}>
      ...
      {diff ? <DiffView diff={diff} filePath={part.filePath} label={part.state === "running" ? "Proposed edit" : "Applied edit"} /> : part.input && <pre className="tool-input mt-3 overflow-x-auto whitespace-pre-wrap text-xs text-muted-foreground">{part.input}</pre>}
