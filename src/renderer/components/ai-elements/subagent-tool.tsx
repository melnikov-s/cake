...
const subagentProjectionSchema = z.object({
...
export function SubagentTool({ part }: { part: Extract<UiPart, { kind: "tool" }> }) {
  ...
  const parallelCount = output?.completed !== undefined && output.total !== undefined ? `${output.completed}/${output.total}` : undefined;
  const title = part.name === "subagent_parallel" ? `parallel delegation${parallelCount ? ` ${parallelCount}` : ""}` : `${profile} subagent`;

  return <div className={`tool-call subagent-call rounded-xl border border-border bg-muted/35 px-4 py-3${part.state === "running" ? " subagent-running" : ""}`}>
    ...
    {recentTools.length > 0 && <div className="subagent-activity" aria-label="Subagent activity">
      {recentTools.map((item, index) => <span key={`${item.name ?? "tool"}-${index}`}><i className={`tool-state tool-${item.state === "error" ? "error" : item.state === "running" ? "running" : "success"}`} />{item.name ?? "tool"}</span>)}
    </div>}
    {(totalTokens !== undefined || cost !== undefined) && <div className="subagent-usage">{totalTokens?.toLocaleString()} tokens{cost !== undefined ? ` · $${cost.toFixed(4)}` : ""}</div>}
    {finalText && <details className="subagent-result"><summary>Result</summary><Markdown className="mt-2 text-xs">{finalText}</Markdown></details>}
  </div>;
}
