  ...
  return (
    <div className="settings-page">
      ...
      <section className="settings-section" aria-labelledby="utility-model-title">
        <header><div><h2 id="utility-model-title">Utility model</h2><p>Runs user-configured, lightweight background tasks such as naming sessions.</p></div><span className="settings-source">Cake</span></header>
        <div className="settings-fields">
          <div className="settings-field"><span>Model<small>When unset, Cake makes no utility calls and session lists use the truncated first message.</small></span><span className="settings-inline-action"><ModelCombobox ariaLabel="Utility model" groups={providerGroups} value={utilityModelValue} onSelect={(value) => void settings.selectUtilityModel(value)} variant="settings" />{utilityModel && <Button variant="ghost" size="sm" type="button" disabled={settings.utilityModelSaving} onClick={() => void settings.clearUtilityModel()}>Clear</Button>}</span></div>
          <label><span>Reasoning<small>The reasoning effort sent with utility requests.</small></span><ThinkingLevelSelect ariaLabel="Utility model thinking level" value={utilityModel?.thinkingLevel ?? "off"} levels={thinkingLevelSchema.options} disabled={!utilityModel || settings.utilityModelSaving} onSelect={(level) => void settings.selectUtilityThinkingLevel(level)} /></label>
        </div>
      </section>
