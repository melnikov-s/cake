...
export const ChangeExplorer = observer(function ChangeExplorer({ store, reviews, browse, chat }: { store: ChangesStore; reviews: ReviewsStore; browse: BrowseStore; chat: MainChatStore }) {
  ...
  if (!change) return <main className="change-explorer-empty"><div><small>{chat.sessionTitle}</small><h1>{store.error ? "Unable to inspect changes" : store.loading ? "Loading session changes…" : "No session changes"}</h1><p>{store.error ?? (store.loading ? "Cake is comparing the session's Git checkpoints." : "The session's latest checkpoint matches its starting checkpoint.")}</p><Button variant="outline" onClick={() => store.close()}>Return to chat</Button></div></main>;
