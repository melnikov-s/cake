...
export function ModelCombobox({ ariaLabel, groups, value, onSelect, variant = "compact" }: ModelComboboxProps) {
  ...
  const openMenu = (initialQuery = "") => {
    ...
    const selectedIndex = initialQuery ? -1 : filteredModels.findIndex((model) => modelValue(model) === value);
  ...
  const onKeyDown = (event: KeyboardEvent<HTMLInputElement>) => {
    ...
    if (event.key === "ArrowDown") {
      ...
      event.preventDefault();
      setActiveIndex((index) => filteredModels.length ? (index - 1 + filteredModels.length) % filteredModels.length : 0);
  ...
  return <div ref={rootRef} className={`model-combobox ${variant}`} onBlur={(event) => {
    ...
    <input
      ...
      aria-activedescendant={open && filteredModels[activeIndex] ? `${listboxId}-option-${activeIndex}` : undefined}
      autoComplete="off"
      spellCheck={false}
      readOnly={!open}
      ...
      onKeyDown={onKeyDown}
    />
    <span className="model-combobox-chevron" aria-hidden="true">⌄</span>
    {open && <div className="model-combobox-popover">
      <div className="model-combobox-list" id={listboxId} role="listbox" aria-label={`${ariaLabel} options`}>
        {filteredGroups.length === 0 ? <p className="model-combobox-empty">No matching models</p> : filteredGroups.map((group) => <div className="model-combobox-group" role="group" aria-label={group.name} key={group.id}>
