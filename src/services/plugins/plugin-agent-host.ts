...
import type { CakeRuntimeEvent } from "../pi/runtime/cake-runtime";
import {
...
export interface PluginAgentDriver {
  readonly openAgent: (input: {
    readonly target: PluginAgentTarget;
  ...
  readonly subscribeAgent: (
    sessionId: string,
    listener: (event: CakeRuntimeEvent) => void,
  ) => () => void;
  readonly agentSnapshot: (sessionId: string) => Promise<SessionSnapshot>;
  readonly agentPrompt: (
    sessionId: string,
    text: string,
    delivery: "prompt" | "steer" | "follow-up",
  ) => Promise<SessionSnapshot>;
  readonly agentAbort: (sessionId: string) => Promise<SessionSnapshot>;
  readonly releaseAgent: (sessionId: string) => void;
}
...
export class PluginAgentHost {
  ...
  constructor(
    private readonly options: {
      ...
      driver(workspacePath: string): PluginAgentDriver;
  ...
  async open(
    ...
    try {
      ...
      const resolvedModel = this.options.resolveModel(
        open.model,
        snapshot,
        this.options.utilityModel(),
      );
      ...
      if (snapshot) driver.releaseAgent(snapshot.sessionId);
      throw error;
    }
  }
  ...
  dispose() {
    for (const [id, handle] of this.handles) this.disposeHandle(id, handle);
    this.privateRefs.clear();
  ...
  private disposeHandle(handleId: string, handle: Handle) {
    ...
    if (handle.privateSession) this.privateRefs.delete(handle.ref.id);
