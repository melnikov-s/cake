...
import {
  cakePluginManifestSchema,
  pluginIdSchema,
  type CakePluginManifest,
  type PluginDiagnostic,
} from "../../plugin/plugin-contract";
import { hasFileErrorCode } from "../../utils/file-errors";
  ...
  for (const item of roots) {
    for (const file of await sourceFiles(item.root)) {
      hash
        .update(item.label)
        .update("\0")
        .update(relative(item.root, file))
        .update("\0")
...
export class PluginRepository {
    ...
    for (const entry of await readdir(this.paths.plugins, { withFileTypes: true })) {
      ...
      try {
        ...
        statuses.push({
          id: entry.name,
          name: manifest.name,
          enabled: manifest.id === entry.name && manifest.enabled,
          renderer: manifest.renderer,
          backend: manifest.backend,
          scene: manifest.scene,
          activeScene: manifest.activeScene,
          diagnostics:
              ...
              : [
                  {
                    phase: "discovery",
                    file: manifestPath,
                    message: `Manifest ID ${manifest.id} must match directory ${entry.name}`,
                  },
                ],
        });
      } catch (error) {
  ...
  async setEnabled(pluginId: string, enabled: boolean) {
    ...
    if (manifest.id !== pluginId)
      throw new Error("Plugin manifest ID does not match its directory");
    await this.writer.write(
      manifestPath,
      `${JSON.stringify({ ...manifest, enabled, activeScene: enabled ? manifest.activeScene : false }, null, 2)}\n`,
    );
  ...
  async setActiveScene(pluginId?: string) {
    const operation = async () => {
      ...
      for (const entry of entries) {
        ...
        if (activeScene) {
          if (!manifest.enabled || !manifest.scene)
            throw new Error(`Plugin ${pluginId} does not provide an enabled scene`);
          selected = true;
        }
        manifests.push({ path, manifest: { ...manifest, activeScene } });
      }
      if (pluginId && !selected)
        throw new Error(`Cake could not find enabled scene plugin ${pluginId}`);
      for (const item of manifests)
        await this.writer.write(item.path, `${JSON.stringify(item.manifest, null, 2)}\n`);
      return this.listPluginStatuses();
    };
    const result = this.authoringPending.then(operation, operation);
    this.authoringPending = result.then(
      () => undefined,
      () => undefined,
    );
    return result;
  ...
  async authoringSnapshot() {
    ...
    for (const item of roots) {
      for (const file of await sourceFiles(item.root))
        files.push(`${item.label}/${relative(item.root, file).split(sep).join("/")}`);
    }
    return {
      workingRevision: await hashRoots(roots),
  ...
  async readPluginFile(pluginId: string, relativePath: string) {
    ...
    if (content.length > 2_000_000)
      throw new Error("Customization source file is too large to read through Cake Chat");
    return content;
  }
  ...
  async writePluginFile(
    ...
    const operation = async () => {
      if (content.length > 2_000_000)
        throw new Error("Customization source file exceeds the 2 MB authoring limit");
      ...
      if (before.workingRevision !== expectedWorkingRevision) {
        throw new Error(
          `Customization working tree changed: expected ${expectedWorkingRevision}, found ${before.workingRevision}`,
        );
      }
  ...
  async createPlugin(options: PluginCreateOptions, expectedWorkingRevision: string) {
    const operation = async () => {
      ...
      if (!options.renderer && !options.backend && !options.scene)
        throw new Error("A plugin must include a renderer, backend, or scene");
      const before = await this.authoringSnapshot();
      if (before.workingRevision !== expectedWorkingRevision)
        throw new Error(
          `Customization working tree changed: expected ${expectedWorkingRevision}, found ${before.workingRevision}`,
      ...
      try {
        await lstat(root);
        throw new Error(`Plugin ${parsedId} already exists`);
      } catch (error) {
  ...
  private async pluginTarget(pluginId: string, relativePath: string, mustExist: boolean) {
    ...
    if (!isWithin(canonicalRoot, sourceRoot) || basename(sourceRoot) !== parsedId)
      throw new Error("Plugin directory does not match its ID");
    ...
    for (const segment of parentSegments) {
      ...
      try {
        ...
        if (entry.isSymbolicLink() || !entry.isDirectory())
          throw new Error(`Plugin path crosses a non-directory entry: ${relativePath}`);
      } catch (error) {
        if (hasFileErrorCode(error, "ENOENT")) break;
        throw error;
      }
    }
    if (!mustExist) return target;
    const canonical = await realpath(target);
    if (!isWithin(sourceRoot, canonical) || !(await lstat(canonical)).isFile())
      throw new Error("Plugin source is not a regular in-root file");
    return canonical;
  }
  ...
  async inspect(): Promise<CustomizationSource> {
    ...
    for (const entry of await readdir(this.paths.plugins, { withFileTypes: true })) {
      ...
      try {
        ...
        if (manifest.id !== entry.name)
          throw new Error(`Manifest ID ${manifest.id} must match directory ${entry.name}`);
        ...
        const resolveEntry = async (kind: "renderer" | "backend" | "scene", entryPath?: string) => {
          ...
          if (!isWithin(canonicalRoot, logicalEntry))
            throw new Error(`Plugin ${kind} entry escapes its directory: ${entryPath}`);
          const resolvedEntry = await realpath(logicalEntry);
          if (!isWithin(canonicalRoot, resolvedEntry) || !(await lstat(resolvedEntry)).isFile())
            throw new Error(
              `Plugin ${kind} entry is not a regular in-directory file: ${entryPath}`,
            );
        ...
        diagnostics.push({
          phase: "discovery",
          file: join(root, "cake-plugin.json"),
          message: error instanceof Error ? error.message : String(error),
        });
      }
    }
    plugins.sort((a, b) => a.manifest.id.localeCompare(b.manifest.id));
    const activeScenes = plugins.filter(
      (plugin) => plugin.manifest.activeScene && plugin.sceneEntry,
    );
    if (activeScenes.length > 1)
      diagnostics.push({
        phase: "discovery",
        message: `Multiple plugins select an active scene: ${activeScenes.map((plugin) => plugin.manifest.id).join(", ")}`,
      });
    const activeScene = activeScenes.length === 1 ? activeScenes[0] : undefined;
    const scene = activeScene?.sceneEntry;

    const agentResources: CustomizationSource["agentResources"] = {
      skills: [],
      prompts: [],
      extensions: [],
    };
    for (const plugin of plugins) {
      for (const [directory, target, directories] of [
        ["skills", agentResources.skills, true],
        ["prompts", agentResources.prompts, false],
        ["pi-extensions", agentResources.extensions, false],
      ] as const) {
        const resourceRoot = join(plugin.root, directory);
        let entries;
        try {
          entries = await readdir(resourceRoot, { withFileTypes: true });
        } catch (error) {
          if (hasFileErrorCode(error, "ENOENT")) continue;
          throw error;
        }
        for (const entry of entries) {
    ...
    const roots = plugins.map((plugin) => ({
      label: `plugin:${plugin.manifest.id}`,
      root: plugin.root,
    }));
    return {
      scene,
      scenePluginId: activeScene?.manifest.id,
      plugins,
      revision: await hashRoots(roots),
      diagnostics,
      agentResources,
    };
  }
  ...
  async snapshotSource(source: CustomizationSource) {
    ...
    try {
      if ((await lstat(destination)).isDirectory()) return destination;
    } catch (error) {
      if (!hasFileErrorCode(error, "ENOENT")) throw error;
    }
    const temporary = `${destination}.tmp-${crypto.randomUUID()}`;
    const roots = source.plugins.map((plugin) => ({
      label: join("plugins", plugin.manifest.id),
      root: plugin.root,
    }));
    try {
      await mkdir(temporary, { recursive: true });
      ...
      const snapshotRoots = source.plugins.map((plugin) => ({
        label: `plugin:${plugin.manifest.id}`,
        root: join(temporary, "plugins", plugin.manifest.id),
      }));
      const snapshotRevision = await hashRoots(snapshotRoots);
      if (snapshotRevision !== source.revision)
        throw new Error(
          `Customization source changed while snapshotting revision ${source.revision}`,
        );
