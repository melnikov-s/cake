...
export function createCakeDrawOperations(control: CakeDrawControl): CakeOperationDefinition[] {
  const operation = <Input>(definition: {
    ...
    guidance: [
      ...
      "Give flowcharts an explicit direction (usually LR for pipelines or TB for hierarchies), keep labels concise, and avoid duplicate edges between the same nodes when one labeled edge communicates the relationship. Cake uses linear Mermaid routes, separates coincident parallel connectors, and layers connectors behind node boxes but above subgraph backgrounds; labels remain above the diagram. Use draw.read and draw.apply for further cleanup.",
