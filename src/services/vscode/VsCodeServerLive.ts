...
export const makeVsCodeServerLive = (options: VsCodeServerLiveOptions) => {
  let activeManager: VsCodeServerManager | undefined;
  const layer: Layer.Layer<VsCodeServer, never, ApplicationState | Electron | ProjectAccess> =
    Layer.effect(
      VsCodeServer,
      Effect.gen(function* () {
  ...
  return {
    layer,
    controller: {
      closeForWindow: (ownerId: number) => activeManager?.closeForWindow(ownerId),
      backToAgentForWindow: (ownerId: number) =>
        activeManager?.backToAgentForWindow(ownerId) ?? false,
    },
  } as const;
};
