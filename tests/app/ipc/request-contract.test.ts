...
describe("cake.request/v1 contract", () => {
    ...
    expect(
      parseRequestInput({
        ...
        view: {
          ...
          language: "react",
          source:
            "export default ({ submit }) => <button onClick={() => submit({choice:'staging'})}>Staging</button>",
        },
      }).view.type,
    ).toBe("widget");
  });
  ...
  it("rejects unknown protocols, views, and missing response schemas", () => {
    expect(() =>
      parseRequestInput({
        ...base,
        protocol: "cake.request/v2",
        view: { type: "form", fields: [] },
      }),
    ).toThrow();
    ...
    expect(() =>
      parseRequestInput({
        ...
        view: { type: "widget", language: "html", source: "<button>Yes</button>" },
      }),
    ).toThrow();
  });
});
