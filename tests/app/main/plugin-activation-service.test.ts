...
const candidate = (revision: string, sourceRevision = revision) => ({ revision, sourceRevision, directory: `/builds/${revision}`, indexHtml: `/builds/${revision}/index.html`, diagnostics: [], backends: [] });
...
function builderFor(buildCandidate: () => Promise<ReturnType<typeof candidate>>) {
  ...
  return { buildCandidate: build, isBuildCurrent: vi.fn(async () => true), repository: { inspect: vi.fn(async () => ({ revision: sourceRevision })) } } as unknown as PluginBuildService;
}
...
describe("PluginActivationService", () => {
  it("requires a render handshake, retains rollback history, and rolls back atomically", async () => {
    ...
    const second = await service.validate(); await service.activateValidated(second.revision, second.sourceRevision); await service.markHealthy(second.revision);
  ...
  it("rejects a stale source base before starting another build", async () => {
    ...
    const service = new PluginActivationService(paths, builder); await service.load();
    const first = await service.validate(); await service.activateValidated(first.revision, first.sourceRevision); await service.markHealthy(first.revision);
  ...
  it("selects factory recovery after an interrupted pending activation", async () => {
    ...
    const first = new PluginActivationService(paths, builder); await first.load();
    const validated = await first.validate(); await first.activateValidated(validated.revision, validated.sourceRevision);
  ...
  it("rebuilds an active customization when its bundled Cake renderer is stale", async () => {
    ...
    (initialBuilder as unknown as { repository: { inspect(): Promise<{ revision: string }> } }).repository = { inspect: async () => ({ revision: sourceRevision }) };
    await initial.activateValidated(first.revision, first.sourceRevision); await initial.markHealthy(first.revision);
