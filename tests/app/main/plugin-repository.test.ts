...
describe("PluginRepository", () => {
  ...
  it("requires at least one executable entry in a v2 manifest", async () => {
    ...
    await writeFile(join(paths.plugins, "empty.plugin", "cake-plugin.json"), JSON.stringify({ schemaVersion: 2, id: "empty.plugin", name: "Empty" }));
    const result = await new PluginRepository(paths).inspect();
    expect(result.plugins).toEqual([]);
  ...
  it("provides revision-checked, in-root source authoring", async () => {
    ...
    await mkdir(plugin, { recursive: true });
    await writeFile(join(plugin, "cake-plugin.json"), JSON.stringify({ schemaVersion: 2, id: "example.authoring", name: "Example Authoring", renderer: "index.tsx" }));
    await writeFile(join(plugin, "index.tsx"), "export default {}\n");
    const repository = new PluginRepository(paths);
    const before = await repository.authoringSnapshot();
    expect(before.files).toContain("plugins/example.authoring/index.tsx");
    const after = await repository.writePluginFile("example.authoring", "index.tsx", "export default { repaired: true }\n", before.workingRevision);
    ...
    await expect(repository.writePluginFile("example.authoring", "other.tsx", "broken", before.workingRevision)).rejects.toThrow("working tree changed");
    await expect(repository.readPluginFile("example.authoring", "../../outside.tsx")).rejects.toThrow("escapes");
  });
  ...
  it("scaffolds plugin-owned capabilities and selects at most one scene", async () => {
    ...
    await repository.createPlugin({ id: "example.scene", name: "Example Scene", renderer: true, backend: false, scene: true }, initial.workingRevision);

    expect(await repository.readPluginFile("example.scene", "cake-plugin.json")).toContain('"activeScene": false');
  ...
  it("serializes optimistic writes so the same revision cannot win twice", async () => {
    ...
    const repository = new PluginRepository(paths);
    const before = await repository.authoringSnapshot();
    await repository.createPlugin({ id: "example.one", name: "One", renderer: true, backend: false, scene: false }, before.workingRevision);
    const current = await repository.authoringSnapshot();
    const results = await Promise.allSettled([
      repository.writePluginFile("example.one", "one.tsx", "export default 1\n", current.workingRevision),
      repository.writePluginFile("example.one", "two.tsx", "export default 2\n", current.workingRevision)
    ]);
  ...
  it("rejects authoring through a plugin-directory symlink", async () => {
    ...
    await expect(repository.writePluginFile("example.escape", "index.tsx", "no", snapshot.workingRevision)).rejects.toThrow("does not match");
  });
  ...
  it("deletes only a validated plugin directory", async () => {
    ...
    await mkdir(plugin, { recursive: true });
    await writeFile(join(plugin, "cake-plugin.json"), JSON.stringify({ schemaVersion: 2, id: "example.removable", name: "Example Removable", renderer: "index.tsx", enabled: true }));
    await writeFile(join(plugin, "index.tsx"), "export default {}\n");
  ...
  it("refuses to delete a plugin-directory symlink", async () => {
    ...
    const outside = join(root, "outside");
    await Promise.all([mkdir(paths.plugins, { recursive: true }), mkdir(outside, { recursive: true })]);
    await writeFile(join(outside, "cake-plugin.json"), JSON.stringify({ schemaVersion: 2, id: "example.escape", name: "Example Escape", renderer: "index.tsx" }));
    await symlink(outside, join(paths.plugins, "example.escape"));
