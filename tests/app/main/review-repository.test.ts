...
describe("ReviewRepository", () => {
  ...
  it("persists only review metadata and projects chat parts from the referenced Pi session", async () => {
    const root = await mkdtemp(join(tmpdir(), "cake-reviews-"));
    directories.push(root);
    ...
    expect(await readFile(repository.reviewContextPath("/project", "session"), "utf8")).toContain(
      "Code: src/app.ts · diff rows 2-3",
    );
    expect(
      (await repository.listSession("/project", "session")).filter(
    ...
    const answered = await repository.completeRun("/project", "session", created.id, runId, {
      sessionId: "pi-review",
      sessionFile: "/reviews/pi-review.jsonl",
    });
    ...
    expect(
      (
        await new ReviewRepository(root, join(root, "pi-sessions"), async () => ({
          parts: projectedParts,
        })).listSession("/project", "session")
      )[0]?.status,
    ).toBe("resolved");
  });

  it("atomically claims pending comments and rejects stale completion", async () => {
    const root = await mkdtemp(join(tmpdir(), "cake-review-claims-"));
    directories.push(root);
    const repository = new ReviewRepository(root, join(root, "pi-sessions"));
    const anchor = {
      path: "src/app.ts",
    ...
    expect(
      await repository.completeRun("/project", "session", created.id, staleRun, {
        sessionId: "stale",
        sessionFile: "/stale.jsonl",
      }),
    ).toBeUndefined();
    expect((await repository.get("/project", "session", created.id))?.pendingComments).toHaveLength(
      1,
    );

    await repository.reply("/project", "session", created.id, "A later comment");
    await repository.completeRun("/project", "session", created.id, winner!.submission!.runId, {
      sessionId: "winner",
      sessionFile: "/winner.jsonl",
    });
    expect((await repository.get("/project", "session", created.id))?.pendingComments).toEqual([
      expect.objectContaining({ body: "A later comment" }),
    ]);
  });

  it("recovers abandoned running claims as retryable failures", async () => {
    const root = await mkdtemp(join(tmpdir(), "cake-review-recovery-"));
    directories.push(root);
    const repository = new ReviewRepository(root, join(root, "pi-sessions"));
    const anchor = {
      path: "src/app.ts",
  ...
  it("keeps a failed comment pending without projecting its failed Pi-session copy", async () => {
    ...
    expect(failed?.parts).toEqual([
      expect.objectContaining({ text: "Are you sure?", deliveryState: "sending" }),
    ]);
    const record = await repository.get("/project", "session", created.id);
    expect(record).toMatchObject({
