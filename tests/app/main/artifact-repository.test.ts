...
describe("ArtifactRepository", () => {
  it("persists content-addressed payloads, enforces revisions, hydrates, and exports fallbacks", async () => {
    ...
    await expect(repository.upsert("/project", { ...base, revision: 3 })).rejects.toThrow(
      "revision must advance",
    );
    const second = await repository.upsert("/project", {
      ...base,
      revision: 2,
      payload: { ...base.payload, rows: [{ id: "one", score: 2 }] },
    });
