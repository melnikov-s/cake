...
function explorerProps(store: ProjectWorkbenchStore) {
  ...
  const chatStore = (threadId: string) => {
    ...
    const chat = mount(
      createStore(ChatStore, {
        id: () => threadId,
        parts: () =>
          (fixture.reviewThreads ?? []).find((thread: { id: string }) => thread.id === threadId)
            ?.uiParts ?? [],
  ...
  const reviews = {
    ...
    get pendingCommentCount() {
      return fixture.pendingReviewCommentCount ?? fixture.pendingReviewThreads?.length ?? 0;
    },
    get draftAnchor() {
      return draftAnchor;
    },
...
function mockReviewThread(input: {
  id: string;
  status: "open" | "resolved";
  pending: boolean;
  anchor: {
    path: string;
    start: { diffLine: number; newLine: number };
    end: { diffLine: number; newLine: number };
  };
  messages: Array<{ id: string; role: "user" | "assistant"; body: string; status: "complete" }>;
}) {
  const uiParts = input.messages.map((message) => ({
    id: message.id,
    kind: "text" as const,
    role: message.role,
    text: message.body,
...
describe("ChangeExplorer", () => {
  ...
  it("fills the app with a highlighted diff and a selectable file tree", () => {
    ...
    expect(container.querySelector(".change-explorer-file header")?.textContent).toContain(
      "Refine the plan",
    );
    ...
    act(() =>
      container.querySelector<HTMLButtonElement>(".change-explorer-tree li button")!.click(),
    );
    expect((store as unknown as Record<string, any>).selectChangeExplorerFile).toHaveBeenCalledWith(
      "src/app.ts",
    );
  });
  ...
  it("scrolls the full diff when a sidebar file is selected", () => {
    ...
    const plan = [
      ...container.querySelectorAll<HTMLButtonElement>(".change-explorer-tree li > button"),
    ].find((button) => button.textContent?.includes("PLAN.md"))!;
    act(() => plan.click());
    act(() => root.render(<ChangeExplorer {...explorerProps(store)} />));
  ...
  it("selects the file whose section reaches the top while scrolling the full diff", () => {
    ...
    Object.defineProperty(diff, "getBoundingClientRect", {
      configurable: true,
      value: () => ({ top: 0 }),
    });
  ...
  it("keeps the full session name available when the header context is truncated", () => {
    const sessionTitle =
      "A very long session name that should stay on a single truncated line in the Changes header";
    const store = {
      workspaceChanges: changes,
      selectedWorkspaceChange: changes[0],
      sessionTitle,
    ...
    await act(async () =>
      container
        .querySelector<HTMLButtonElement>(
          '.change-explorer-view-toggle button[aria-pressed="false"]',
        )!
        .click(),
    );

    expect(readWorkspaceFile).toHaveBeenCalledWith("src/app.ts");
    expect(container.querySelector('[aria-label="Full file src/app.ts"]')?.textContent).toContain(
      "const fresh = true;",
    );
    expect(
      container.querySelectorAll(".change-explorer-full-file .change-explorer-line"),
    ).toHaveLength(3);
    expect(container.querySelector(".change-explorer-full-file .remove")?.textContent).toContain(
      "−const old = true;",
    );
    expect(container.querySelector(".change-explorer-full-file .add")?.textContent).toContain(
      "+const fresh = true;",
    );
    expect(container.querySelector(".change-explorer-full-file .context")?.textContent).toContain(
      "const unchanged = true;",
    );

    act(() =>
      container
        .querySelector<HTMLButtonElement>(".change-explorer-view-toggle button:first-child")!
        .click(),
    );
    expect(container.querySelector('[aria-label="Changes to src/app.ts"]')).not.toBeNull();
  });
  ...
  it("adds a comment from a line in the full file view", async () => {
    ...
    const store = {
      ...
      selectChangeExplorerFile: vi.fn(),
      closeChangeExplorer: vi.fn(),
      readWorkspaceFile: vi.fn(async () => "const fresh = true;\nconst unchanged = true;"),
    } as unknown as ProjectWorkbenchStore;
    act(() => root.render(<ChangeExplorer {...explorerProps(store)} />));
    await act(async () =>
      container
        .querySelector<HTMLButtonElement>(".change-explorer-view-toggle button:last-child")!
        .click(),
    );
    ...
    act(() =>
      container
        .querySelector<HTMLButtonElement>(
          '.change-explorer-full-file .review-gutter button[aria-label="Comment on line 2"]',
        )!
        .click(),
    );
  ...
  it("opens the same inline comment composer from a gutter line", () => {
    ...
    const add = container.querySelector<HTMLButtonElement>(
      '.review-gutter button[aria-label="Comment on line 1"]',
    )!;
    act(() => add.click());
    expect(container.querySelector(".review-thread-draft")).not.toBeNull();
    expect(
      container.querySelector<HTMLTextAreaElement>('textarea[aria-label="Message code chat"]')
        ?.placeholder,
    ).toBe("Ask Cake about this code…");
  ...
  it("waits for the plus button and uses the current selection as the first message", () => {
    ...
    act(() => root.render(<ChangeExplorer {...explorerProps(store)} />));
    const row = [...container.querySelectorAll<HTMLElement>(".change-explorer-line")].find((item) =>
      item.textContent?.includes("const fresh = true;"),
    )!;
    ...
    expect(
      container.querySelector(".review-thread-draft .user-message")?.textContent,
    ).not.toContain("const fresh = true;");
  });
  ...
  it("uses the first Escape to close a comment composer without escaping the changes view", () => {
    ...
    act(() =>
      container
        .querySelector<HTMLButtonElement>('.review-gutter button[aria-label="Comment on line 1"]')!
        .click(),
    );

    act(() =>
      container
        .querySelector<HTMLTextAreaElement>('textarea[aria-label="Message code chat"]')!
        .dispatchEvent(
  ...
  it("saves comments with Enter and leaves Shift+Enter available for a new line", async () => {
    ...
    const buttonTextarea = container.querySelector<HTMLTextAreaElement>(
      'textarea[aria-label="Message code chat"]',
    )!;
    act(() => {
      Object.getOwnPropertyDescriptor(HTMLTextAreaElement.prototype, "value")!.set!.call(
  ...
  it("minimizes a review thread when it is resolved", () => {
    const thread = mockReviewThread({
      ...
      anchor: {
        path: "src/app.ts",
        start: { diffLine: 1, newLine: 1 },
        end: { diffLine: 1, newLine: 1 },
      },
      messages: [
    ...
    const resolve = [
      ...container.querySelectorAll<HTMLButtonElement>(".review-thread header button"),
    ...
    expect(container.querySelector(".review-thread-collapsed")?.textContent).toContain(
      "Review thread",
    );
  });
  ...
  it("toggles an unresolved review thread from its header without resolving it", () => {
    ...
    expect(
      [...container.querySelectorAll<HTMLButtonElement>(".review-thread header button")].some(
  ...
  it("submits a thread reply from the button or Enter and keeps Shift+Enter for a newline", async () => {
    ...
    await act(async () =>
      container
        .querySelector<HTMLButtonElement>(".chat-embedded-composer button[type=submit]")!
        .click(),
    );
  ...
  it("does not expose the removed pending-comment batch action", () => {
    ...
    const store = {
      ...
      sessionTitle: "Review",
      reviewThreads: [],
      pendingReviewThreads: [{ id: "review-1" }],
      pendingReviewCommentCount: 2,
      reviewThreadStreaming: vi.fn(() => false),
      createReviewThread: vi.fn(async () => undefined),
      replyReviewThread: vi.fn(async () => undefined),
      resolveReviewThread: vi.fn(async () => undefined),
  ...
  it("lists review threads in the sidebar and links each one to its inline thread", () => {
    ...
    const thread = mockReviewThread({
      ...
      messages: [
        { id: "message-1", role: "user", body: "Can you explain this?", status: "complete" },
      ],
    });
    const store = {
      workspaceChanges: changes,
      ...
      selectChangeExplorerFile: vi.fn(),
      closeChangeExplorer: vi.fn(),
      sendPendingReviewComments: vi.fn(async () => undefined),
      focusReviewThread,
    } as unknown as ProjectWorkbenchStore;
    act(() => root.render(<ChangeExplorer {...explorerProps(store)} />));

    expect(container.querySelector(".review-navigation")).toBeNull();
    expect(container.querySelector(".review-thread-index")?.textContent).toContain(
    ...
    expect(
      container
        .querySelector<HTMLElement>(".change-explorer-sidebar-body")
        ?.style.getPropertyValue("--review-panel-height"),
    ).toBe("120px");
    act(() =>
      container.querySelector<HTMLButtonElement>(".review-thread-index li button")!.click(),
    );
  ...
  it("puts resolved threads last and renders them in the muted state", () => {
    ...
    const store = {
      ...
      sessionTitle: "Review",
      reviewThreads: [
        makeThread("resolved-1", "resolved", "Finished comment"),
        makeThread("open-1", "open", "Active comment"),
      ],
    ...
    const rows = [
      ...container.querySelectorAll<HTMLButtonElement>(".review-thread-index li button"),
    ];
    expect(rows.map((row) => row.textContent)).toEqual([
      expect.stringContaining("Active comment"),
      expect.stringContaining("Finished comment"),
    ]);
