...
import { findFileMention, SlashCommandCombobox } from "../../../src/renderer/components/slash-command-combobox";
...
describe("SlashCommandCombobox", () => {
  ...
  it("finds unquoted and quoted file mentions at the caret", () => {
    expect(findFileMention("check @src/app then", 14)).toMatchObject({ token: "@src/app", prefix: "src/app" });
    expect(findFileMention('check @"my folder/app', 21)).toMatchObject({ token: '@"my folder/app', prefix: '"my folder/app' });
  ...
  it("shows Pi file suggestions and inserts the active file with Enter", async () => {
    ...
    function ControlledCombobox() {
      ...
      return <SlashCommandCombobox aria-label="Message" commands={[]} suggestFiles={suggestFiles} value={value} onValueChange={setValue} onSubmit={vi.fn()} />;
    }
    act(() => root.render(<ControlledCombobox />));
    ...
    expect(container.querySelector('[role="listbox"]')?.getAttribute("aria-label")).toBe("Project files");
    expect(container.querySelector('[role="option"][aria-selected="true"]')?.textContent).toContain("src/app.ts");
  ...
  it("keeps directory completions open and preserves Pi quoting for paths with spaces", async () => {
    const suggestFiles = vi.fn(async (prefix: string) => prefix === "src/"
  ...
  it("keeps the file menu open while a shorter replacement query is pending", async () => {
    let resolveReplacement!: (value: Array<{ value: string; label: string; description: string }>) => void;
    const suggestFiles = vi.fn((prefix: string) => prefix === "alpha"
      ? Promise.resolve([{ value: "@alpha.ts", label: "alpha.ts", description: "alpha.ts" }])
      : new Promise<Array<{ value: string; label: string; description: string }>>((resolve) => { resolveReplacement = resolve; }));
    function ControlledCombobox() {
    ...
    await act(async () => { resolveReplacement([{ value: "@app.ts", label: "app.ts", description: "app.ts" }]); await Promise.resolve(); });
    expect(container.querySelector('[role="option"]')?.textContent).toContain("app.ts");
  });

  it("ignores stale file-search responses", async () => {
    let resolveFirst!: (value: Array<{ value: string; label: string; description: string }>) => void;
    let resolveSecond!: (value: Array<{ value: string; label: string; description: string }>) => void;
    const suggestFiles = vi.fn((prefix: string) => new Promise<Array<{ value: string; label: string; description: string }>>((resolve) => {
    ...
    await act(async () => { resolveSecond([{ value: "@beta.ts", label: "beta.ts", description: "beta.ts" }]); await Promise.resolve(); });
    await act(async () => { resolveFirst([{ value: "@alpha.ts", label: "alpha.ts", description: "alpha.ts" }]); await Promise.resolve(); });
