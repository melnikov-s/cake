...
describe("plugin command runtime", () => {
  it("uses friendly aliases when unique and qualified names on collision", async () => {
    ...
    definePlugin({
      id: "test.alpha",
      contributions: {},
      commands: { open: { description: "Open alpha", run: first } },
    });
    expect(pluginCommandSnapshot().some((command) => command.name === "open")).toBe(true);
    definePlugin({
      id: "test.beta",
      contributions: {},
      commands: { open: { description: "Open beta", run: second } },
    });
    const names = pluginCommandSnapshot().map((command) => command.name);
    expect(names).toContain("test.alpha.open");
    expect(names).toContain("test.beta.open");
    expect(names).not.toContain("open");
    expect(await dispatchPluginCommand("/test.beta.open tomorrow")).toBe(true);
    expect(second).toHaveBeenCalledWith(
      "tomorrow",
      expect.objectContaining({ signal: expect.any(AbortSignal), reveal: expect.any(Function) }),
    );
  });
  ...
  it("bridges command reveal requests to the active scene", async () => {
    ...
    definePlugin({
      ...
      commands: {
        show: {
          description: "Show",
          run: (_args, context) => context.reveal("test.reveal.panel", { tab: 2 }),
        },
      },
    });
    ...
    expect((listener.mock.calls[0]![0] as CustomEvent).detail).toEqual({
      contributionId: "test.reveal.panel",
      input: { tab: 2 },
    });
  ...
  it("renders automatically registered slot contributions in deterministic order", () => {
    ...
    expect(renderToStaticMarkup(createElement(Slot, { name: "project-session.right.top" }))).toBe(
