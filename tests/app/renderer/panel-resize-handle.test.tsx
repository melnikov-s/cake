...
describe("PanelResizeHandle", () => {
  ...
  it("resizes from the keyboard and exposes separator values", () => {
    ...
    act(() => root.render(<PanelResizeHandle label="Resize sidebar" value={292} min={220} max={500} edge="left" onChange={onChange} />));
  ...
  it("reverses horizontal movement for a panel attached to the right edge", () => {
    const onChange = vi.fn();
    act(() => root.render(<PanelResizeHandle label="Resize details" value={320} min={240} max={500} edge="right" onChange={onChange} />));
    const handle = container.querySelector<HTMLElement>('[role="separator"]')!;
