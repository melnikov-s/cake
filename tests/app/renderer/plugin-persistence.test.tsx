...
describe("plugin persistence hooks", () => {
  ...
  it("hydrates through Suspense before dependent effects run", async () => {
    ...
    const request = vi.fn(() => new Promise((resolve) => { resolveLoad = resolve; }));
    ...
    const container = document.createElement("div"); containers.push(container); document.body.appendChild(container);
    const root = createRoot(container);
    act(() => root.render(<Suspense fallback={<i>loading</i>}><Probe /></Suspense>));
    expect(container.textContent).toBe("loading");
    expect(effect).not.toHaveBeenCalled();

    await act(async () => resolveLoad({ type: "plugin-state", record: { value: "stored", version: 3 } }));
