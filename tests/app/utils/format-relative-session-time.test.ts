...
describe("formatRelativeSessionTime", () => {
  ...
  it("uses relative labels for recent session activity", () => {
    expect(formatRelativeSessionTime(new Date(now - 20 * 60_000).toISOString(), now)).toBe(
      "20 min ago",
    );
    expect(formatRelativeSessionTime(new Date(now - 2 * 3_600_000).toISOString(), now)).toBe(
      "2 hrs ago",
    );
    expect(formatRelativeSessionTime(new Date(now - 2 * 86_400_000).toISOString(), now)).toBe(
      "2 days ago",
    );
  });
