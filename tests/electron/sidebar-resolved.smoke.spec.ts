...
test("resolves and restores the selected project session in the desktop sidebar", async () => {
  ...
  await Promise.all([mkdir(userData, { recursive: true }), mkdir(project, { recursive: true }), mkdir(sessionDirectory, { recursive: true })]);
  ...
  await writeFile(join(sessionDirectory, `${sessionId}.jsonl`), [
    ...
    { type: "message", id: "user-1", parentId: null, timestamp, message: { role: "user", content: [{ type: "text", text: "Implement the resolved sidebar" }], timestamp: 0 } }
  ...
  try {
    ...
    const selectedSession = page.locator(".session-item.active").filter({ has: page.locator(".session-resolve-action") });
