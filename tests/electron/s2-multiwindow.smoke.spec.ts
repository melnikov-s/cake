...
test("keeps window state independent across Pi sessions", async () => {
  ...
  await Promise.all([
    writeFile(
      ...
      JSON.stringify({ ...base, draft: "first window draft" }),
    ),
    writeFile(
      join(userData, "window-state-1.json"),
      JSON.stringify({ ...base, draft: "second window draft" }),
    ),
    writeFile(
  ...
  try {
    ...
    await expect(second.getByLabel("Message")).toHaveValue("second window draft", {
      timeout: 20_000,
    });
    ...
    await expect
      .poll(() => second.locator(".workspace").getAttribute("data-session-id"))
