  ...
  try {
    ...
    const validateAndActivate = async (input: Record<string, unknown> = {}) => {
      ...
      await request({
        ...
        request: input.request ?? "Smoke test activation",
      });
    };
    ...
    const filesV1 = (await request({ type: "list-plugin-files" })) as {
      workingRevision: string;
      files: string[];
    };
    expect(filesV1.files).toContain("plugins/smoke.example/index.tsx");
    expect(
      await request({ type: "read-plugin-file", pluginId: "smoke.example", path: "index.tsx" }),
    ).toMatchObject({ content: expect.stringContaining("PLUGIN_V1") });
    ...
    const filesV2 = (await request({
      type: "write-plugin-file",
      pluginId: "smoke.example",
      path: "index.tsx",
      content: sourceV2,
      expectedWorkingRevision: filesV1.workingRevision,
    })) as { buildRevision: string };
    await validateAndActivate({
      expectedBaseRevision: sourceV1.state.sourceRevision,
      expectedSourceRevision: filesV2.buildRevision,
      request: "Upgrade smoke plugin to V2",
    });
    ...
    const filesBeforeCrash = (await request({ type: "list-plugin-files" })) as {
      workingRevision: string;
    };
    const broken = (await request({
      ...
      pluginId: "smoke.example",
      path: "scene.tsx",
      content: `export default function Broken() { throw new Error("PLUGIN_RUNTIME_CRASH"); }\n`,
      expectedWorkingRevision: filesBeforeCrash.workingRevision,
    })) as { buildRevision: string };
    await validateAndActivate({
      expectedBaseRevision: activeV2.state.sourceRevision,
      expectedSourceRevision: broken.buildRevision,
      request: "Exercise runtime recovery",
    });
    ...
    await expect
      .poll(async () => request({ type: "list-plugins" }))
      .toMatchObject({
        plugins: [expect.objectContaining({ id: "smoke.example", enabled: false })],
      });
  } finally {
