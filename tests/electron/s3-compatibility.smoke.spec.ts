...
test("surfaces Pi resources and adapts extension dialogs and legacy widgets", async () => {
  ...
  await writeFile(join(userData, "window-state.json"), JSON.stringify({ projectPath: project, recentProjectPaths: [project], trustedProjectPaths: [project], draft: "", theme: "system", thinkingExpanded: false }));
